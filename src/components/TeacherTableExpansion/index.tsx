...
export default function TeacherTable({
    ...
    useEffect(() => {
        const filtered = allData.filter((item) => {
            ...
            return matchSchool && matchSubject && matchTimeline
        })

        onFilterChange(filtered)
        onPageChange(1) // Reset to first page on filter change
    }, [searchSchool, searchSubject, searchTimeline, allData, onFilterChange, onPageChange])
                        ...
                        {data.length > 0 ? (
                            data.map((item) => (
                                <tr key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                                    <td className="px-6 py-4 text-sm text-foreground font-medium">{item.schoolName}</td>
                                    <td className="px-6 py-4 text-sm text-foreground">
                                        <span className="inline-block bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-medium">
                                            {item.subject}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-muted-foreground">{item.timeline}</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
